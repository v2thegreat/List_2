...
class list_2:
	...
	def __init__(self, iterable = []):
		...
		self.arr = arange(len(iterable), dtype=object) if len(iterable) else array([])
	...
	def __iadd__(self, value):  
		...
		temp_lst=arange(len(self)+len(value), dtype=object) #Creating an empty array of length=len(self)+len(y)
		...
		self.arr=arange(len(self)+len(value), dtype=object) #Making self.arr an empty array of length=len(self)+len(y)
	...
	def __mul__(self, mul_value): #Multiplication function in lists
		...
		temp_lst=arange(len(self)*mul_value, dtype=object) #Creating a placeholder array of the appropriate size
